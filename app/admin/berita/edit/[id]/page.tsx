...
function EditBeritaPage() {
  ...
  const loadBerita = async () => {
    try {
      setLoadingData(true);
      const response = await apiClient.getBeritaById(params.id as string);
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          tags: Array.isArray(berita.tags) ? berita.tags.join(", ") : "",
          terbit: berita.terbit,
          tanggalTerbit: berita.tanggalTerbit
            ? new Date(berita.tanggalTerbit).toISOString().slice(0, 16)
            : "",
        });
      }
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Gagal memuat data berita";
      alert(errorMessage);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.updateBerita(params.id as string, submitData);
      router.push("/admin/berita");
    } catch (error) {
      const errorMessage =
