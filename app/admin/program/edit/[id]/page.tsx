...
function EditProgramPage() {
  ...
  const loadProgram = async () => {
    try {
      setLoadingData(true);
      const response = await apiClient.getProgramById(params.id as string);
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          realisasi: program.realisasi?.toString() || "",
          status: program.status,
          tanggalMulai: new Date(program.tanggalMulai)
            .toISOString()
            .slice(0, 10),
          tanggalSelesai: program.tanggalSelesai
            ? new Date(program.tanggalSelesai).toISOString().slice(0, 10)
            : "",
          lokasiKegiatan: program.lokasiKegiatan || "",
          penanggungJawab: program.penanggungJawab || "",
          targetPenerima: program.targetPenerima || "",
          terbit: program.terbit,
        });
      }
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Gagal memuat data program";
      alert(errorMessage);
    ...
    try {
      const submitData = {
        ...
        realisasi: formData.realisasi
          ? parseFloat(formData.realisasi)
          : undefined,
        tanggalSelesai: formData.tanggalSelesai || undefined,
      };

      await apiClient.updateProgram(params.id as string, submitData);
      router.push("/admin/program");
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Gagal memperbarui program";
      alert(errorMessage);
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-1">Perbarui informasi program</p>
            </div>
            <Link
              href="/admin/program"
...
export default withAuth(EditProgramPage);
